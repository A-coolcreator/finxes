...
def parse_text_flow_engine(pdf_path, bank_signature=None):
    ...
    if bank_signature == "BOB":
        ...
        # Remove any stray rows that are clearly incomplete.
    ...
    # ------------------------------------------
        ...
        for idx_cursor, line_idx in enumerate(data_indices):
            ...
            if idx_cursor > 0:
                ...
                # Canara.pdf starts with 2,45,708.00 opening balance
    ...
    # ------------------------------------------
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages:
                ...
                # Group words into lines vertically
                lines_dict = {}
                for w in words:
                ...
                # Process each line sorted vertically
                ...
                for y in sorted_ys:
                    ...
                    # Check if starting with date DD-MM-YYYY
                    if len(line_words) < 3:
                        continue
                    ...
                    # Build column fields based on x0 coordinates
                    txn_date = ""
                    val_date = ""
                    desc_parts = []
                    ref_no = ""
...
def parse_tabular_grid_engine(pdf_path):
    ...
    best_strategy = None # Caches 'lines' or 'text' strategy after first match
    
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            # 1. Fast text pre-check: skip pages with no transaction markers to avoid slow table extractions
            ...
            # 2. Extract tables using cached strategy, or determine the best one
            tables = []
            if best_strategy == "lines":
                ...
                if tables:
                    ...
                    # Fallback to spacing clustering
                    tables = page.extract_tables({"vertical_strategy": "text", "horizontal_strategy": "text"})
                    if tables:
                        best_strategy = "text"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if upi_signature:
            print(f"[->] UPI/Wallet {upi_signature} statement detected. Launching specialized UPI Parser...")
        ...
        if df_statement is not None and not df_statement.empty:
            ...
            destination_name = f"{base_filename}.csv"
            destination_path = os.path.join(output_folder, destination_name)
            
            df_statement.to_csv(destination_path, index=False)
            print(f"[OK] Parsing Complete! Saved {len(df_statement)} rows into: {destination_path}")
        else:
            print("[FAIL] Error: Transactions could not be extracted. Verify your PDF has an active text layer.")
